...
class _LoginPageState extends State<LoginPage> {
  ...
  Future _loginUser({User user}) async {
    if (_formKey.currentState.validate()) {
      try {
        ...
        await loginUser(http.Client(), widget.prefs, user.toJson()).then((onValue) {
          if (onValue) {
            widget.prefs.setBool(PrefConstants.ISLOGGEDIN, true);
            widget.prefs.setString(PrefConstants.LOGGED_EMAIL, user.email);
            Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (BuildContext context) => ViewPrices(
                      prefs: widget.prefs,
                    )));
          } else {
